...
struct VersionCheckAppState {
    ...
    private func checkForUpdatesAsync(_ manualCheck: Bool = false) async throws -> Void {
        do {
            ...
            if Bundle.main.releaseVersionOlder(than: versionManifest.iPad.current) {
                DispatchQueue.main.async {
                    withAnimation {
                        versionCheckToast.wrappedValue = VersionCheckToast(
                            appVersion: Bundle.main.releaseVersionNumberPretty,
                            currentVersion: versionManifest.iPad.current,
                            linkToCurrentVersion: versionManifest.iPad.link)
                        if versionManifest.iPad.vital {
